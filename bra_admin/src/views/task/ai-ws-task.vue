...
<template>
      ...
      <div class="switch_bar">
        <div class="consun_list handel_area">
          ...
          <el-table-column prop="img_url" label="图片" minWidth="120">
            ...
            <el-table-column prop="ptime" :label="$t('sys_p009')" minWidth="120">
              <template slot-scope="scope">
                <div>{{ scope.row.ptime > 0 ? $baseFun.resetTime(scope.row.ptime * 1000) : "-" }}</div>
              </template>
            </el-table-column>
          </el-table>
  ...
  import { getaimessagelist,getsysconfig,upsysconfig,doaimessage,dooutexcel,getaimessagewslist,doaimessagews } from '@/api/task'
  ...
  export default {
    ...
    methods: {
      ...
      getTaskList(num) {
        ...
        getaimessagewslist(params).then(res => {
          this.loading = false;
