                    ...
                    <div :class="['ws_head w_f flex-item flex-dir-c',!show_code?'ws_head_active':'']">
                        <div class="task_award w_f">
                            <div class="w_f task_book flex-item flex-between font_28">
                                <div class="task_title"> {{$t('home_191')}} </div>
                                <van-button class="flex-item" @click="show_code=!show_code">{{show_code?$t('home_171'):$t('home_170')}}</van-button>
                            </div>
                        </div>
                    </div>
...
import { getaimessagetaskcontacts,gettgaimsginfo } from '@/api/task';
import { getautogroupinfo,getqrcodetg,delaccount,submitautogrouptask,getautogroupaccountstatus,gettgaccountlist,getwsincomelist,submittgcode } from '@/api/home'
export default {
name: 'scan_online',
...
methods: {
    ...
    getVerifBtn(){
        this.isLoading = true;
        getqrcodetg({area_code:this.current_code,account:this.tg_account}).then(res => {
    ...
    submitVerif(){
        if(!this.tg_account)return this.$toast(this.$t("other_001",{value:this.$t("login_038")})); 
        this.verLoading = true;
        let prams = {
            account:this.tg_account,
            code:this.tg_verfy,
            area_code:this.current_code,
            // qr_code: this.code_info.qr_code,
            // ip_id: this.code_info.ip_id,
            // type: this.code_info.type,
            // host: this.code_info.host,
            // port: this.code_info.port,
            // user: this.code_info.user,
            // pwd: this.code_info.user
        }
        submittgcode(prams).then(res => {
    ...
    initWechatList(){
        ...
        gettgaccountlist({page:this.l_page,limit:5}).then(res => {
    ...
    async getIncomeList(){
        this.ref_loading = true;
        let res = await gettgaimsginfo({task_type:1});
        this.loading = false;
        if(res.code) return;
        this.link = res.link;
    ...
    handleCode(code){
        this.current_code = code.replace(/[^\d.]/g, '');
    ...
    async submitBtn(){
        ...
        this.sLoading=true;
        let backRes = await submitautogrouptask({task_info_id:this.task_id,account:`${this.current_code}${this.tg_account}`});
    ...
    jumpTgSend(row){
        ...
        let textMsg =`https://t.me/Liang009988?text=${encodeURIComponent(row.content)}`
        if(this.$Helper.checkBrowser()){
