<template>
  <div class="earn">
    <div class="custom_head">
      <page-header :bgColor="true" :show-icon="true" :title="$t('mine_010')"></page-header>
    ...
    <div v-if="list&&list.length>0" class="record_list w_f">
      <van-list v-model="loading" :finished="finished" :finished-text="$t('other_063')" :loading-text="$t('other_029')"
                offset="60" @load="onLoad">
        <div v-for="(item,index) in list" :key="index" class="record_item flex-item flex-align">
          <img alt="" class="l_icon" src="@/assets/images/mine/order_icon.png">
          <div class="w_f flex-item flex-align flex-dir-c">
            <div class="w_f flex-item flex-align flex-between">
              <div class="flex-item flex-dir-c">
                <p class="task_bonus font_30">{{
                    profitType.find(val => val.value == item.type) ? profitType.find(val => val.value == item.type).lable : ""
                  }}</p>
                <p v-if="item.task_type>0" class="task_type font_24">{{ taskOption[item.task_type] }}</p>
              </div>
              <div v-if="item.type==9" class="task_money font_30" style="color:#F52C2C;">{{ item.amount }}</div>
              <div v-else-if="item.type==4&&!checkReduce(item.amount)" class="task_money font_30">{{
                  item.amount
                }}
              </div>
              <div v-else-if="item.type==8&&checkReduce(item.amount)" class="task_money font_30">+{{
                  item.amount
                }}
              </div>
              <div v-else-if="item.type==8&&!checkReduce(item.amount)" class="task_money font_30"
                   style="color:#F52C2C;">{{ item.amount }}
              </div>
              <div v-else class="task_money font_30">+{{ item.amount }}</div>
            </div>
    ...
    <div v-else class="empty_tips w_f flex-item flex-align flex-center flex-dir-c">
      <img alt="" src="../../assets/images/empty_icon.png"/>
      <p>{{ $t('tail_010') }}</p>
    </div>
    <van-overlay :show="showState" @click="showState = false">
      <div class="screen_down" @click.stop>
        <div class="w_f flex-item flex-dir-c">
          ...
          <ul>
            <li v-for="item in profitType" :key="item.value" :class="stateValue===item.value?'checkActive':''"
          ...
          <ul>
            <li v-for="(item,index) in profitTime" v-show="item!=''" :key="index"
                :class="index === timeValue  ? 'checkActive':''" @click="changeTime(item,index)">{{ item }}
            </li>
          </ul>
        </div>
        <div class="footer_btn w_f flex-item flex-between">
          <van-button :loading="isLoading" type="primary" @click="submitFun(1)">{{ $t('other_053') }}</van-button>
          <van-button :loading="isLoading" type="primary" @click="submitFun(2)">{{ $t('home_038') }}</van-button>
        </div>
      </div>
    </van-overlay>
  </div>
...
export default {
  ...
  computed: {
    ...
    profitType() {
      return [
        {lable: this.$t('other_052'), value: -1},
        {lable: this.$t('other_025'), value: 1},
        {lable: this.$t('other_026'), value: 2},
        {lable: this.$t('spre_023'), value: 3},
        {lable: this.$t('spre_024'), value: 4},
        {lable: this.$t('spre_025'), value: 5},
        {lable: this.$t('spre_026'), value: 6},
        {lable: this.$t('other_027'), value: 8},
        {lable: this.$t('other_028'), value: 9},
        {lable: this.$t('other_104'), value: 10},
        {lable: this.$t('other_105'), value: 11}
      ];
    },
    taskOption() {
  ...
  methods: {
    ...
    filterPay(type) {
      let result = this.profitType.filter(row => {
