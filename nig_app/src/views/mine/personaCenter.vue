<template>
  <div class="person_c">
    <page-header :bgColor="true" :show-icon="true" :title="$t('pay_023')"></page-header>
    <div class="person_content">
      <div class="user_info w_f flex-item flex-content-l">
        <div class="lable_text">{{ $t('pay_046') }}</div>
        <van-radio-group v-model="countryType" class="w_f flex-item flex-dir-r">
          <van-radio name="Nigeria" shape="square" @click="changeCountryType('Nigeria')">{{ $t('pay_047') }}</van-radio>
          <van-radio name="India" shape="square" @click="changeCountryType('India')">{{ $t('pay_048') }}</van-radio>
        </van-radio-group>
      </div>
      <!-- 尼日利亚 -->
      <div v-if="countryType==='Nigeria'" class="user_info w_f flex-item flex-content-l">
        <div class="lable_text">{{ $t('pay_038') }}</div>
        <van-radio-group v-model="curIndex" class="w_f flex-item flex-dir-r" @change="changeType">
      ...
      <template v-if="countryType==='Nigeria'&&curIndex==1">
        <div class="user_info">
          ...
          <van-field v-model="collectCard" :border="false"
                     :placeholder="$t('other_001',{value:$t('pay_006')})" oninput="value=value.replace(/[^\d]/g,'')"/>
        </div>
        <div class="user_info bank_account" @click="showBank(0)">
          ...
          <div class="flex-between">
            <van-field
                v-model="bankName"
                :border="false"
                :placeholder="$t('other_014',{value:$t('pay_021')})"
                :readonly=true />
            <van-icon :style="{transform: `rotate(${accountModel == true ? 90 : 0}deg)`}" name="arrow"
                      style="transition: all .3s linear"/>
          </div>
        </div>
        <div class="user_info">
          <span class="lable_text">{{ $t('pay_022') }}</span>
          <van-field v-model="collectName" :border="false" :placeholder="$t('pay_022')"/>
        </div>
      </template>
      <template v-else-if="countryType==='Nigeria'">
        <div class="user_info alipay_info">
          <span class="lable_text">{{ $t('pay_041') }}</span>
          <van-field v-model="collectTRX" :border="false" :placeholder="$t('other_001',{value:$t('pay_041')})"
      ...
      <template v-if="countryType==='India'">
        ...
        <div class="user_info">
          <span class="lable_text">{{ $t('pay_022') }}</span>
          <van-field v-model="collectName" :border="false" :placeholder="$t('other_001',{value:$t('pay_022')})"/>
        </div>
        <div class="user_info">
          <span class="lable_text">{{ $t('pay_052') }}</span>
          <van-field v-model="identifyNum" :border="false" :placeholder="$t('other_001',{value:$t('pay_052')})"/>
        </div>
      </template>
      ...
      <van-action-sheet v-model="accountModel" :actions="bankOption" :cancel-text="$t('other_007')" :round="false"
                        @close="accountModel=false" @select="changeAccount"/>
      <div class="button_area">
        <van-button :loading="isLoading" type="primary" @click="submitBtn">{{ $t('other_015') }}</van-button>
      </div>
