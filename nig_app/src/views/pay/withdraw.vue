<template>
  <div class="withdraw">
    <div class="panel">
      <page-header :bgColor="true" :show-icon="true" :title="$t('mine_002')"></page-header>
        ...
        <div class="card_box">
          <div class="card flex-item flex-dir-c">
            ...
            <div class="amount van-ellipsis flex-item flex-align">
              ...
              <div v-if="country ==='Nigeria'" class="flex-item flex-align">
                {{ WithdMoney || 0 }}≈{{ curIndex == 2 ? income_trx || 0 : income_naira || 0 }}
                <span>{{ curIndex == 2 ? $t('pay_040') : $t('pay_024') }}</span>
              </div>
              <div v-if="country ==='India'" class="flex-item flex-align">
                {{
                  WithdMoney || 0
                }}≈{{ curIndex == 2 ? income_trx || 0 : income_rupia || 0 }}<span>{{
                  curIndex == 2 ? $t('pay_040') : $t('pay_045')
                }}</span>
              </div>
            </div>
        ...
        <div class="share_box w_f flex-item flex-between">
          <div class="share_link flex-item flex-align">
            ...
            <span class="account_text"
                  v-text="card_no&&curIndex==1||card_no&&curIndex==2?card_no:$t('pay_042',{value:$t('login_038')})"></span>
          </div>
    ...
    <van-popup v-model="showModel" :close-on-click-overlay="false" :overlay="true">
      <div class="dialog_content">
        ...
        <van-cell-group :border="false" inset
          ...
          <van-field v-model="withdraw_num" clearable oninput="value=value.replace(/[^\w_]/g,'')" placeholder="0"
                     type="number"/>
        </van-cell-group>
        <div v-if="country ==='Nigeria'" class="auto_computed">
          <span v-if="curIndex==2">{{ withdraw_num || 0 }}{{ $t('pay_027') }}≈{{ withdraw_num * ratio_trx }}{{
              $t('pay_040')
            }}</span>
          <span v-else>{{ withdraw_num || 0 }}{{ $t('pay_027') }}≈{{ withdraw_num *ratio_naira }}{{ $t('pay_024') }}</span>
        </div>
        <div v-if="country ==='India'" class="auto_computed">
          <span v-if="curIndex==2">{{ withdraw_num || 0 }}{{ $t('pay_027') }}≈{{ withdraw_num * ratio_trx }}{{
              $t('pay_040')
            }}</span>
          <span v-else>{{ withdraw_num || 0 }}{{ $t('pay_027') }}≈{{ withdraw_num *ratio_rupia }}{{ $t('pay_045') }}</span>
        </div>
        <div class="custom_dialog__footer">
          ...
          <van-button :loading="isLoading" :loading-text="$t('other_012')" class="custom_dialog_confirm"
                      @click="withdrawBtn">{{ $t('other_003') }}
          </van-button>
        </div>
      </div>
    </van-popup>
  ...
  methods: {
    syncInitApi() {
      ...
      Promise.all([fun1, fun2]).then(res => {
        const [{income, income_naira, income_rupia, income_trx,ratio_naira,ratio_rupia,ratio_trx}, {
          limit_amount,
          limit_count,
          limit_count_status,
          india_limit_amount
        ...
        this.ratio_naira = ratio_naira ||0 //金钱对应奈拉的比率
    ...
    async withdrawBtn() {
      ...
      if (this.country ==='Nigeria') {
        minAmount = this.withdraw_cash
        typeNumber = Number(this.curIndex)
        bankCode = this.bank_code
      }else if (this.country ==='India'){
        minAmount = this.india_limit_amount
        typeNumber = 0
        bankCode =''
      }
      ...
      } else if (this.withdraw_num < minAmount) {
        return this.$toast(this.$t('pay_028', {value: minAmount}));
      }
